...
sendRequest (uint16_t content,double size,uint16_t clientId,uint16_t serverId,uint16_t pol)
{
  ...
  if(pol==4 or pol==7)
  {
    std::string filename = "python3 src/dash-migration/AHP/AHP.py " + directory +" "+ToString(simulationId) +" "+ToString(size)+" "+ToString(serverId)+" "+ToString(content);
  ...
  if (pol==5)
  {
    std::string filename = "python3 src/dash-migration/Guloso-Aleatorio/exemplo.py " + directory +" "+ToString(simulationId) +" "+ToString(size)+" "+ToString(serverId)+" "+ToString(content)+" "+"guloso";
    std::string ahp = execute(filename.c_str());
    std::vector <std::string> Ahp;
    //system(filename.c_str());
    //std::string ahp="5 3";
  ...
  if (pol==6)
  {
    std::string filename = "python3 src/dash-migration/Guloso-Aleatorio/exemplo.py " + directory +" "+ToString(simulationId) +" "+ToString(size)+" "+ToString(serverId)+" "+ToString(content)+" "+"ale";
    std::string ahp = execute(filename.c_str());
    std::vector <std::string> Ahp;
    //system(filename.c_str());
    Ahp = split(ahp.c_str(), " ");
