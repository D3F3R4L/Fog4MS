...
def main():
  ...
  for k in numberOfClients:
    ...
    #print(os.listdir())
  print(Cost)
  barGraph(Cost,costConf,name='cost.pdf',xlabel='Nº de Microserviços',ylabel='Custo por hora(USD)',xticks=numberOfClients,labels=['Fog4MS','Guloso','Aleatório','Nuvem'])
  print(playback)
  barGraph(migration,migrationConf,'migration.pdf','Nº de Microserviços','Tempo de migração (s)',xticks=numberOfClients,labels=['Fog4MS','Guloso','Aleatório','Nuvem'])
  barGraph(buffering,bufferingConf,'buffering.pdf','Nº de Microserviços','Tempo de bufferização (s)',xticks=numberOfClients,labels=['Fog4MS','Guloso','Aleatório','Nuvem'])
  stackedBarGraph(playback,'start.pdf','Nº de Microserviços','Tempo de início do Video (s)',xticks=numberOfClients,labels=['Fog4MS','Guloso','Aleatório','Nuvem','Tempo de Migração', 'Tempo de bufferização'])
  plotGraph(fair, 'usage.pdf','Nº de Microserviços','Distribuição da carga (Jain Fairness)',xticks=numberOfClients,labels=['Fog4MS','Guloso','Aleatório','Nuvem'])
  barGraph(numberTotal,numberTotalConf,'number.pdf','Nº de Microserviços','Microserviços na névoa',xticks=numberOfClients,labels=['Fog4MS','Guloso','Aleatório'])#,'Nuvem'])
  print(fair)
  print(numberTotal)
  print('Finished')
...
def fogStatus():
  ...
  while i<runs:
    ...
    while j<len(files):
      ...
      #else:
      #  onOff=onOff*0.00034
      if(name!='throughputServer_sim{simu}_1.0.17.1_Log.csv'.format(simu=simulation)):
        val=content[600].split("/")
        #val=list(set(val))
