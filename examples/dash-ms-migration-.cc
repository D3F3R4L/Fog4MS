...
{
  std::string filename = "python3 src/dash-migration/Delays.py " + dirTmp +" "+ToString(simulationId);
...
main (int argc, char *argv[])
{
  ...
  p2p.SetDeviceAttribute ("DataRate", StringValue ("100Mb/s")); // This must not be more than the maximum throughput in 802.11n
  p2p.SetDeviceAttribute ("Mtu", UintegerValue (1500));
  p2p.SetChannelAttribute ("Delay", StringValue ("3.5ms"));
...
SPAddress = fogAddress[7];
  ...
  {
    V4PingHelper ping = V4PingHelper (fogAddress[i]);
  ...
  for (NodeContainer::Iterator i = networkNodes.Begin (); i != networkNodes.Begin ()+numberOfServers; ++i)
  {
    servers.Add (*i);
  }
  //servers.Add(PA);
  ...
  for (uint i = 0; i < numberOfClients; i++)
  {
    ...
    quali=qualis[quali];
    segmentSizeFilePath = "src/dash-migration/dash/segmentSizesBigBuck"+ToString(quali)+".txt";
